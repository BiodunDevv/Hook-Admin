          ...
          <div className="grid w-full gap-5 px-4 py-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-5">
              ...
              <FieldSection icon={MapPin} title="Operating location" description="Connect this market to an enabled Operating State, city, and compatible Dispatch Hub.">
                ...
                <div className="space-y-1.5 sm:col-span-2"><Label>Dispatch Hub <span className="font-normal text-muted-foreground">(optional)</span></Label><Select value={form.hubId || "none"} onValueChange={(value) => set("hubId", value === "none" ? "" : value)} disabled={!form.stateId || hubs.isLoading}><SelectTrigger className="w-full"><SelectValue placeholder="Select Hub" /></SelectTrigger><SelectContent><SelectItem value="none">Assign later</SelectItem>{hubOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select>{form.stateId && !hubs.isLoading ? <p className="text-xs text-muted-foreground">{hubOptions.length ? `Showing Hubs in ${selectedState || "this State"} only.` : `There are no Dispatch Hubs in ${selectedState || "this State"} yet. You can assign one later.`}</p> : null}</div>
