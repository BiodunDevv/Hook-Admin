    ...
    <ScrollArea className="flex-1 pr-2">
      <div className="space-y-5">
        ...
        {transactions.map((tx) => {
          ...
          return (
          <div key={tx.id} className="flex items-center justify-between rounded-md p-1">
            <div className="flex items-center gap-3">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${iconBg} ${iconColor}`}>
                <Icon size={20} />
              </div>
              <div>
                <p className="mb-0.5 font-semibold leading-tight text-zinc-900">{tx.vendor?.businessName || "Legacy settlement"}</p>
                <p className="text-[12px] text-zinc-500">{tx.status.replaceAll("_", " ")} historical liability</p>
              </div>
            </div>
