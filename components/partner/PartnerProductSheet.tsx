...
function variantLabel(variant: Variant) {
  return [variant.size, displayColorName(variant.colour)].filter(Boolean).join(" · ") || "Default";
}
...
export function PartnerProductSheet({
  ...
  const setVariantId = setChosenVariantId;
                ...
                <div className="flex flex-wrap gap-2">
                  {variants.map((variant) => (
                    <button
                      ...
                      className={`rounded-md border px-3 py-2 text-[14px] transition ${
