...
interface Booth { id: string; name: string; description?: string; isActive: boolean; boothType: string; location?: { address?: string; lat?: number; lng?: number }; previewImageUrl?: string; }
...
export default function BoothDetailPage() {
  ...
  return (
    <div className="space-y-4 px-3 py-3 sm:px-5">
      ...
      {booth && <Card className="rounded-lg shadow-none"><CardContent className="grid gap-4 p-4 text-sm md:grid-cols-2"><div><p className="text-muted-foreground">Status</p><StatusBadge status={booth.isActive ? "Active" : "Inactive"} /></div><div><p className="text-muted-foreground">Type</p><p>{booth.boothType}</p></div><div><p className="text-muted-foreground">Coordinates</p><p>{booth.location?.lat || 0}, {booth.location?.lng || 0}</p></div><div><p className="text-muted-foreground">Description</p><p>{booth.description || "No description"}</p></div></CardContent></Card>}
    </div>
  );
}
