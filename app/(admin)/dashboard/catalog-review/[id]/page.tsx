...
export default function CatalogReviewDetailPage() {
  ...
  return <div className="w-full space-y-5 px-4 py-5"><PageHeader title={item?.basicTitle || "Catalog submission"} description={item ? `${item.publicId} · Immutable Market Associate capture` : "Review Market Associate-captured catalog evidence"} actions={<><Button variant="outline" size="sm" onClick={() => router.back()}><ArrowLeft /> Back</Button>{item ? <CatalogStatusBadge status={item.status} /> : null}</>} />
